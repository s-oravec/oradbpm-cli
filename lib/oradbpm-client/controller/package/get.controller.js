...
var createChildDependencyTreeNodes = function (packageDependencyTreeNode, packageRepositoryService) {
  return _.reduce(packageDependencyTreeNode.packageVersionDefinition.parsedDependencies, function (acc, pkgDependency, pkgName) {
    acc.push(packageRepositoryService.get(pkgName)
      .then(function (packageDefinition) {
        return new PackageDependencyTreeModel.PackageDependencyTreeNode(packageDependencyTreeNode, pkgDependency, packageDefinition);
      })
    );
    return acc;
  }, []);
...
var get = function (pkgReferences, options) {
  ...
  return createPackageDependencyTreeRoot(oraDBPMClient.packageFileServiceFactory, options)
    ...
    .then(function (packageDependencyTreeRoot) {
      return populateDependencies(deploymentPlan, packageDependencyTreeRoot, oraDBPMClient.packageRepositoryService)
    })
    .then(function () {
      debug(deploymentPlan);
      // TODO: resolve DeploymentPlan packageProposals to packagesResolution
      return Bluebird.resolve();
    })
    .then(function () {
